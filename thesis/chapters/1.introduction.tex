...
While Shor's solution (known as Shor's code) proves that QEC can be theoretically implemented, it is unable to effectively mitigate error due to the large number of additional data and ancilla qubits that it adds as distance scales, and the number of qubits monitored by each detector. This added complexity of the code causes more errors than it can prevent, specially under realistic noise models. 
...
Second, our objective is to quantify the performance of various decoding strategies, from classical \textbf{Projection decoders} to state-of-the-art \textbf{Möbius} and \textbf{Concatenated MWPM} approaches. Using advanced simulation and sampling tools such as \texttt{stim} \cite{gidney2021stim} and \texttt{sinter}, in Chapter \ref{chap:comparative_analysis} we will compare their efficiency through standard metrics: physical error thresholds ($p_{th}$), logical failure rate ($p_L$) scaling as a function of physical noise, theoretical sub-threshold suppression limits, and the trade-off between decoding accuracy and computational time.

\notatfm{In quantum physics, \textbf{decoherence} is the process by which a system loses its quantum properties (like phase or superposition) due to continuous interactions with the environment. By assuming that all logical operations and measurements are perfectly instantaneous, the code-capacity model effectively isolates this environmental decoherence—which corrupts data qubits as they sit idle in the quantum memory—from the operational noise introduced by the physical execution of circuits.} Finally, as we will discuss in Chapter \ref{chap:modeling_quantum_error}, various sources of noise affect real quantum hardware. While realistic hardware-aware simulations of QEC codes require accounting for all of them—including imperfect gates and measurement errors—this study focuses strictly on \textbf{code-capacity noise}. This foundational model isolates the decoherence of data qubits over time by assuming perfect syndrome extraction. Adopting this approach allows us to evaluate the strengths of the 4.8.8 lattice and the algorithmic efficiency of the decoders, without the confounding variables introduced by circuit-level operations. 

Similarly, different error types (X, Y, Z) can appear in a quantum circuit, and the rations between them may differ (some types may be more likely). We will simulate both \textbf{bit-flip} and \textbf{depolarizing} noise, with the former being the standard in the literature and the one we will use to compare our results.
...
\centering
\resizebox{\textwidth}{!}{
    \input{figures/1.introduction/1-classical_repetition.tex}
}
\caption[Mechanism of a classical 3-bit repetition code.]{Mechanism of a classical 3-bit repetition code correcting a single bit-flip error.}
\label{fig:1:classical_repetition}
\end{figure}
...
\begin{figure}[hbt!]
\centering
    \input{figures/1.introduction/1-parity_check.tex}
\caption[A parity check bit appended to a 3-bit message.]{A parity check bit appended to a 3-bit message. The parity bit (0) is calculated to ensure that the total number of '1's in the data block is even.}
\label{fig:1:parity_check}
\end{figure}
...
\begin{figure}[hbt!]
\centering
\input{figures/1.introduction/1-bloch_sphere.tex}
\caption[Geometric representation of a qubit on the Bloch sphere.]{Geometric representation of a qubit on the Bloch sphere. The parameters $\theta$ and $\phi$ define the continuous orientation of the state vector $|\psi\rangle$ relative to the computational basis states $|0\rangle$ and $|1\rangle$.}
\label{fig:1:bloch_sphere}
\end{figure}
\end{enumerate}
...
\end{quantikz}
\caption[Encoding circuit for the bit-flip code.]{Encoding circuit for the bit-flip code. Entanglement distributes the logical information without violating the no-cloning theorem.}
\label{fig:1:bit_flip_encoding}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{quantikz}
    \caption[Syndrome measurement for bit-flip code.]{Syndrome measurement: The ancilla qubits are activated when the two qubits that they monitor are different. Depending on which of the two ancilla qubits are activated, we can determine which of the qubits has flipped. This is known as a Z-type detector}
    \label{fig:1:3-qubit-syndrome}
\end{figure}

As shown in Figure \ref{fig:1:3-qubit-syndrome}, measuring the ancilla qubits does not provide information about the state of each individual qubit, but it provides information about two qubits being different. The following table shows how the measuring of these ancilla qubits maps to specific identified errors.
...
\end{quantikz}
\caption[Encoding circuit for the 3-qubit phase-flip code.]{Encoding circuit for the 3-qubit phase-flip code. The information is first distributed via entanglement, and then Hadamard gates transform the final state into the $\{|+\rangle, |-\rangle\}$ basis.}
\label{fig:1:phase_flip_encoding}
\end{figure}

\notatfm{To measure the relative phases of the data qubits (implementing a X-type detector), the phase-kickback technique is used: the ancilla is put on a $|+\rangle$ state and CNOT gates are run with the ancilla as control qubit.}
...
% Figure \ref{fig:1:3-qubit-phase-syndrome} shows the corresponding circuit.
...
%     \end{quantikz}
%     \caption[Phase-flip syndrome measurement.]{Phase-flip syndrome measurement: The ancilla qubits are initialized in the $\ket{+}$ state using Hadamard gates. To perform the measurement in the $X$ basis, CNOT gates are applied in reverse (ancilla as control, data as target) to exploit phase kickback. A final Hadamard gate returns the ancillas to the computational basis before measurement.}
%     \label{fig:1:3-qubit-phase-syndrome}
% \end{figure}
...
\begin{figure}[hbt!]
    ...
    \begin{quantikz}
        ...
        \lstick{$|0\rangle$} & \qw & \qw & \qw & \qw & \targ{} & \qw & 
    \end{quantikz}
    \caption[Full encoding circuit for the 9-qubit Shor code.]{Full encoding circuit for the 9-qubit Shor code. The initial CNOT and Hadamard gates encode the logical state into a 3-qubit phase-flip code. Subsequently, each of these three qubits is encoded into a 3-qubit bit-flip code, resulting in the final 9-qubit state.}
    \label{fig:1:shor_9qubit_circuit}
\end{figure}
...
\begin{table}[hbt!]
    ...
    \end{tabularx}
    \caption[Ancilla qubits to extract the syndrome in the 9-qubit Shor code.]{Ancilla qubits to extract the syndrome in the 9-qubit Shor code.}
...
\tablenote{It is important to note that phase-flip correction can be applied to any qubit of the cluster for which we detected the error. This is because the phase is encoded in the relative phase between the $|000\rangle$ and $|111\rangle$ components of the cluster, which changes by changing the phase of any of the three qubits. To correct the Y error, the combination of X and Z can be used.}
    ...
    \bottomrule
    \end{tabularx}
    \caption[Decoding the syndrome in the 9-qubit Shor code.]{Decoding the syndrome in the 9-qubit Shor code.}
...
We can \textbf{measure} a stabilizer operator. Such measurement is implemented as a parity check on the data qubits that are connected to this operator, with the result being reflected in the state of an ancilla qubit:
\begin{itemize}
...
\subsubsection{Logical Operators and Code Construction}

Once the stabilizer group $\mathcal{S}$ is defined, we need a way to manipulate the encoded quantum information without leaving the protected logical subspace. This is achieved through \textbf{logical operators}. 

A logical operator must commute with all the stabilizers in $\mathcal{S}$, ensuring that applying it does not trigger any error detection alarms (it leaves the state in the $+1$ eigenspace of all stabilizers). However, unlike stabilizers, non-trivial logical operators are not part of $\mathcal{S}$ itself. Furthermore, pairs of logical operators acting on the same logical qubit (such as a logical $X_L$ and a logical $Z_L$) must anti-commute with each other, mimicking the standard behavior of physical Pauli operators.

A crucial property of the stabilizer formalism is that logical operators are not unique. Because any stabilizer $S \in \mathcal{S}$ acts identically as the identity operator on valid codewords, we can multiply any logical operator $L$ by any stabilizer $S$ to obtain a new operator $L' = L S$. Both $L$ and $L'$ will have the exact same effect on the logical state. We say that $L$ and $L'$ are \textbf{equivalent logical operators}.

This multiplicity leads to a rigorous definition of a code's robustness. The \textbf{distance} of a stabilizer code is defined as the minimum weight (the minimum number of physical qubits affected) among all possible non-trivial logical operators. In other words, out of all equivalent ways to perform a logical operation, the distance is the size of the "shortest" one. If an error chain happens to match this shortest logical operator, it will perform a valid logical operation without triggering any syndrome, effectively corrupting the data silently.

This implies that we cannot just pick any random combination of commuting Pauli strings to build a quantum code. A bad choice of $n-k$ generators might result in a code with a very small distance (e.g., $d=1$ or $d=2$), meaning it provides little to no protection.

To systematically select a generator group that guarantees the required commutation and anti-commutation rules and achieves a high distance $d$, researchers map the problem to mathematical structures. These frameworks allow us to generate families of codes that can be scaled to arbitrary distances systematically. Prominent examples include \textbf{topological codes} (where qubits and stabilizers are arranged in geometric lattices, such as the Surface Code or the Color Code that we will explore in this work) and codes based on algebraic structures with commutative properties, such as polynomial rings used in \textbf{quantum Low-Density Parity-Check (qLDPC)} codes.
...
\begin{itemize}
    \item If the physical error rate, $p$ is below a certain value, $p_{th}$, which depends on the correction protocol itself, increasing the distance reduces the logical noise. That is, the benefit of increasing the code capacity is greater than the drawback of increasing the circuit overhead.
