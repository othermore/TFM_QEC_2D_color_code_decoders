...
\subsubsection{Bit-flip noise}
A special type of biased noise is one where only X-type errors can occur. This is specially relevant in our case as bit-flip only, code capacity models are the standard in the literature to compare across different decoders. The bit-flip noise model is specially adequate when only X-type errors can affect the observable (i.e. the final observable is measured in the computational basis).
...
Under this model, the syndrome extracted by the ancilla qubits is always 100\% reliable. Because measurements never fail, multiple rounds of the simulation process (i.e. analyzing the error caused through multiple idle cycles of the quantum computer) are equivalent to one single round at a higher physical noise level. Therefore, ubder this model the decoder only needs to solve a 2D spatial problem (i.e. matching the errors on the lattice, with no time dimension). 
