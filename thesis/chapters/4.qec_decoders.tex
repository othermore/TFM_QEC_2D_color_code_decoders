...
This exploration is intuitively visualized using expanding regions. The algorithm begins by simultaneously growing a "bubble" outwards from every active defect. These bubbles expand across the adjacent data qubits in the graph, to other non-active defects. When the boundaries of two expanding bubbles collide, the two defects are temporarily matched. The algorithm assumes that the path traversed by the bubbles corresponds to the chain of physical errors that connected them.
