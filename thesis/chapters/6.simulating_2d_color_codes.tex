...
Figure \ref{fig:6:circuit_d3} illustrates the resulting quantum circuit for a distance 3 code with a single measurement round. Due to its significant width, the image has been dynamically split into two halves for readability.
\begin{figure}[htbp]
    \centering
    % Primera mitad (izquierda)
    \begin{subfigure}[b]{\textwidth}
        \centering
        \begin{adjustbox}{trim=0 0 {0.5\width} 0, clip, max width=\textwidth}
            \includegraphics{figures/chapter_6/circuit_d3.pdf}
        \end{adjustbox}
        \caption{First half: Initialization and early stabilizer interactions.}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{\textwidth}
        \centering
        \begin{adjustbox}{trim={0.5\width} 0 0 0, clip, max width=\textwidth}
            \includegraphics{figures/chapter_6/circuit_d3.pdf}
        \end{adjustbox}
        \caption{Second half: Final stabilizer interactions and data measurement.}
    \end{subfigure}
    \caption{Complete quantum circuit for the 4.8.8 Color Code ($d=3$, 1 round). The circuit is generated automatically by mapping the abstract geometric lattice faces into synchronized parity checks.}
