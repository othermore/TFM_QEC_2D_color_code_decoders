...
The grid was evaluated starting from $p=0.02$. The simulation was bounded to a maximum of $25 \times 10^6$ shots per configuration, terminating early as soon as $400$ logical errors were observed. 

Furthermore, to gain a deeper insight into the topological capabilities of the decoder, we ran separate full evaluations for both standard Depolarizing noise and pure X-only (bit-flip) noise.

The results of this evaluation are presented in Figure \ref{fig:projection_threshold}. Using Kernel Density Estimation (KDE) to find the continuous mode of the crossing points (thereby naturally filtering out early finite-size intersection anomalies), we observe an empirical threshold of $p_{th} \approx \text{XX.X}\%$ for the Depolarizing noise model, and $p_{th} \approx \text{YY.Y}\%$ for the pure X noise model. 

Below this crossover point, the logical error curves exhibit a robust exponential suppression across all distances. To obtain a finer resolution of the exact transition phase, Figure \ref{fig:projection_threshold_zoom} displays an amplified high-resolution sweep of the sub-threshold region. During this zoom phase, the early stopping condition was quadrupled to $1600$ logical errors per data point, completely smoothing out any statistical noise.

\begin{figure}[htpb]
    \centering
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter_6/s3_projection_results_threshold.png}
    \end{minipage}\hfill
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter_6/s3_projection_results_X_threshold.png}
    \end{minipage}
    \caption[Simulation 3: Projection decoder thresholds.]{Main threshold evaluation for the topological Projection Decoder. (Left) Performance under standard depolarizing noise showing a threshold of $p_{th} \approx \text{XX.X}\%$. (Right) Performance under a pure X-only noise model showing a threshold of $p_{th} \approx \text{YY.Y}\%$.}
...
\begin{figure}[htpb]
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter_6/s3_projection_results_threshold_zoom.png}
    \end{minipage}\hfill
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/chapter_6/s3_projection_results_X_threshold_zoom.png}
    \end{minipage}
    \caption[Simulation 3: Projection decoder threshold zooms.]{High-resolution evaluation centered precisely around the estimated thresholds to confirm the crossover behavior devoid of finite-size statistical fluctuations.}
    \label{fig:projection_threshold_zoom}
\end{figure}
