...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \includegraphics[width=0.85\linewidth]{figures/c2_torus_colorcode.png}
        \caption{}
    \end{subfigure}
    \hfill
    ...
    \label{fig:color_code_logical_observables}
\end{figure}
...
These strings and nets define the logical operators. When two non-contractible nets of $X$ and $Z$ operators intersect at an odd number of data qubits, they anticommute, forming a valid logical qubit. Because of this rich combinatorial structure, a color code is twice as dense as a surface code. While a surface code wrapped on a torus natively encodes $k=2$ logical qubits, a color code on the exact same toroidal surface encodes $k=4$ logical qubits (see Figure \ref{fig:color_code_logical_observables}). For planar implementations, triangular patches are typically used, which encode $k=1$ logical qubit but offer remarkably different boundary symmetries compared to the square patches of surface codes (see Figure \ref{fig:planar_color_code}).
