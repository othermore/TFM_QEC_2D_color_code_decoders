...
\begin{figure}[htpb]
    \centering
    \input{figures/2.state_of_the_art/2-bbc_construction.tex}
    \caption[Construction of Bivariate Bicycle Codes.]{Construction of Bivariate Bicycle Codes. The data qubits are divided into two identical blocks (Left and Right). The $X$-checks are constructed by connecting to qubits in the Left block according to polynomial $A$ and in the Right block according to polynomial $B$. The $Z$-checks mirror this connectivity but transposed, ensuring that the intersection between any $X$ and $Z$ check is always an even number of qubits, thus guaranteeing they commute while maintaining a strictly bounded weight (LDPC).}
