...
The code is located in the \texttt{src} directory. Each chapter has its own directory and python notebook. The specific code snippets are located in separate files and included in the notebook. 

Libraries and resources required for multiple experiments are located in the directory \texttt{src/lib}.
\end{notebox}
...
\begin{figure}[hbt!]
    ...
    \caption{Logical error for a basic repetition code using the MWPM decoder}
...
\lstinputlisting[language=Python, caption={Basic repetition code simulation}, label={lst:stim_rep_code}]{../src/chapter_5/s1_stim_rep_code.py}
...
\lstinputlisting[language=Python, caption={Implementation of a simple Majority Vote decoder for a repetition code}, label={lst:majority_vote_decoder}]{../src/chapter_5/s2_majority_vote_decoder.py}

As seen in Figure \ref{fig:stim_rep_vote_plot}, for simple repetition codes under basic noise models, both decoders perform similarly, effectively reducing physical errors.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.67\linewidth]{figures/stim_rep_vote_plot.png}
    \caption{Logical error for a basic repetition code using our Majority Vote decoder}
    \label{fig:stim_rep_vote_plot}
\end{figure}
