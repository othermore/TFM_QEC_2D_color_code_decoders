...
First, the circuit is declared and initialized, followed by the application of the necessary gates to encode the logical state and extract the stabilizer operators. A crucial step in custom \texttt{stim} circuits is explicitly tracking the outcomes: parity measurements must be labeled as \texttt{DETECTOR}s (Listing \ref{lst:stim_detector}), while the logical flips are tracked using \texttt{OBSERVABLE\_INCLUDE} targets (Listing \ref{lst:stim_boundary_detector_and_observable}). This strict labeling is what allows the simulator to internally separate syndrome extraction from logical failure evaluation. 
...
\includesnippet[caption={Simulation 6: Adding a noisy gate for circuit-level noise.}, label={lst:stim_noisy_gate}]{S6_SNIPPET_NOISY_GATE}{../src/chapter_5/s6_shor_9_code.py}

\includesnippet[caption={Simulation 6: Adding a noisy measurement for phenomenological noise.}, label={lst:stim_noisy_measurement}]{S6_SNIPPET_NOISY_MEASUREMENT}{../src/chapter_5/s6_shor_9_code.py}

\includesnippet[caption={Simulation 6: Adding code-capacity noise at each round. Each round represents an idle time where the circuit is required to preserve its logical state. In each round, all the detectors are evaluated to determine the errors that were added in this round.}, label={lst:stim_noisy_round}]{S6_SNIPPET_ROUND_NOISE}{../src/chapter_5/s6_shor_9_code.py}

\includesnippet[caption={Simulation 6: Labeling circuit detectors. After measuring a Stabilizer Operator, we label the detector (and we do that with respect to the same detector in prior round) so stim can determine the syndrome.}, label={lst:stim_detector}]{S6_SNIPPET_MEASURE_DETECTORS}{../src/chapter_5/s6_shor_9_code.py}

\includesnippet[caption={Simulation 6: Final measurement of the data qubits and subsequent labeling of Boundary Detectors and the observable.}, label={lst:stim_boundary_detector_and_observable}]{S6_SNIPPET_FINAL_MEASUREMENT}{../src/chapter_5/s6_shor_9_code.py}
