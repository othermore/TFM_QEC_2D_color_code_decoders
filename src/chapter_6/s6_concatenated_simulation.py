...
def run_simulate(distances, execution_mode, allowed_noise_types):
    ...
    for noise_type in allowed_noise_types:
        output_file = f"s6_concatenated_results_{noise_type}.csv" if noise_type != 'depolarizing' else "s6_concatenated_results.csv"
...
def run_plot(distances_filter, allowed_noise_types):
    ...
    for noise_type in allowed_noise_types:
        ...
        plot_threshold(csv_path, title=f"Logical noise vs physical noise - 4.8.8 Color Code (Concatenated MWPM Decoder{title_suffix})", distances_filter=distances_filter)
        
        print(f"Generating Execution Time Plot for {noise_type}...")
        plot_execution_time(csv_path, title=f"Decoding Complexity - Concatenated MWPM Decoder{title_suffix}", distances_filter=distances_filter)
...
def run_zoom(distances, execution_mode, allowed_noise_types):
    ...
    for noise_type in allowed_noise_types:
        base_output_file = f"s6_concatenated_results_{noise_type}.csv" if noise_type != 'depolarizing' else "s6_concatenated_results.csv"
        ...
        est_threshold = calculate_and_save_threshold(base_csv_path, f'CONCAT_MWPM_{noise_type.upper()}', filtered_distances_list, df_filtered)
        ...
        title_suffix = "" if noise_type == 'depolarizing' else " - Pure X Noise"
        plot_threshold_zoom(base_csv_path, est_threshold, title=f"Logical noise vs physical noise - 4.8.8 Color Code (Concatenated MWPM Decoder - Zoom{title_suffix})", distances_filter=distances)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Concatenated MWPM Decoder simulation pipeline.")
    ...
    print("=====================================================")
    print("      Concatenated MWPM Decoder Simulation - 4.8.8 Color Code   ")
    print("=====================================================")
