...
def run_plot(distances_filter):
    ...
    for noise_type in ['depolarizing', 'X']:
        ...
        plot_threshold(csv_path, title=f"Logical noise vs physical noise - 4.8.8 Color Code (Restriction Decoder{title_suffix})", distances_filter=distances_filter)
...
def run_zoom(distances, execution_mode):
    ...
    for noise_type in ['depolarizing', 'X']:
        ...
        est_threshold = calculate_and_save_threshold(base_csv_path, f'RESTRICTION_{noise_type.upper()}', filtered_distances_list, df_filtered)
        ...
        title_suffix = "" if noise_type == 'depolarizing' else " - Pure X Noise"
        plot_threshold_zoom(base_csv_path, est_threshold, title=f"Logical noise vs physical noise - 4.8.8 Color Code (Restriction Decoder - Zoom{title_suffix})", distances_filter=distances)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Restriction Decoder simulation pipeline.")
