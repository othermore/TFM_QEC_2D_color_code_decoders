...
\notatfm{The threshold theorem, proven by Aharonov and Ben-Or \cite{aharonov1997fault}, states that arbitrarily long quantum computation is only possible if the physical error rate is strictly below a certain critical value, known as the threshold.}
...
First, as we will formally define in Chapter \ref{chap:state_of_the_art}, color codes can be implemented through seceral geometries (4.8.8, 6.6.6 and 4.6.12). In this work, we will specifically adopt a \textbf{4.8.8 lattice geometry}. This choice is motivated by it's lower qubit requirements (fewer qubits are needed per distance) and its properties as a doubly-even code, which enables transversal implementation of the full Clifford group, including $H$, $S^\dagger$ and $\text{CNOT}$ \cite{bombin_martindelgado_2006}.
...
\begin{figure}[hbt!]
\centering
\begin{tikzpicture}[line cap=round, line join=round, >=Stealth, scale=3.5]
    \tikzstyle{axis} = [->, thick, gray!80]
    \tikzstyle{back_axis} = [dotted, very thin, gray!40] 
    ...
    \draw[gray!30, loosely dotted, very thin] (0,0) ellipse (1 and 0.25); 
    ...
    \node[draw, fill=white, rounded corners, align=left, font=\small] at (1.3, 0.8) {
...
\begin{enumerate}
    ...
    \item \textbf{Correction:} A code can to correct up to $t$ physical errors:
    \begin{equation}
